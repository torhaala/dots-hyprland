...
const WorkspaceContents = (count = 10) => {
    return DrawingArea({
        ...
        attribute: {
            ...
            workspaceGroup: 0,
            updateMask: (self) => {
                const offset = Math.floor((Hyprland.active.workspace.id - 1) / count) * WORKSPACES_SHOWN;
            ...
            .on('draw', Lang.bind(area, (area, cr) => {
                ...
                for (let i = 1; i <= count; i++) {
                    ...
                    // Moving to
                        ...
                        // cr.setSourceRGBA(mix(activefg.red, wsfg.red, Math.abs(activeWs - i)), mix(activefg.green, wsfg.green, Math.abs(activeWs - i)), mix(activefg.blue, wsfg.blue, Math.abs(activeWs - i)), activefg.alpha);
                        cr.setSourceRGBA(mix(activefg.red, wsfg.red, 1 - Math.abs(activeWs - i)), mix(activefg.green, wsfg.green, 1 - Math.abs(activeWs - i)), mix(activefg.blue, wsfg.blue, 1 - Math.abs(activeWs - i)), activefg.alpha);
                    }
                    // Moving from
                    else if ((i == Math.floor(activeWs) && Hyprland.active.workspace.id > activeWs) || (i == Math.ceil(activeWs) && Hyprland.active.workspace.id < activeWs)) {
                        cr.setSourceRGBA(mix(activefg.red, wsfg.red, 1 - Math.abs(activeWs - i)), mix(activefg.green, wsfg.green, 1 - Math.abs(activeWs - i)), mix(activefg.blue, wsfg.blue, 1 - Math.abs(activeWs - i)), activefg.alpha);
                    ...
                    // Inactive, occupied
                    else if (area.attribute.workspaceMask & (1 << i)) {
                        // Set color for text
                    ...
                    if (area.attribute.workspaceMask & (1 << i)) {
                        ...
                        const occupiedIndicatorRadius = indicatorRadius / 6.5;
                        const occupiedIndicatorGap = occupiedIndicatorRadius;
                        cr.arc(wsCenterX, wsCenterY + indicatorRadius - occupiedIndicatorGap - occupiedIndicatorRadius, occupiedIndicatorRadius, 0, 2 * Math.PI);
                        cr.fill();
                    }
                }
            }))
...
export default () => EventBox({
    ...
    child: Box({
        ...
        children: [Box({
            ...
            css: 'min-width: 2px;',
            children: [WorkspaceContents(WORKSPACES_SHOWN)],
        })]
    }),
    setup: (self) => {
        ...
        self.on('motion-notify-event', (self, event) => {
            ...
            const wsId = Math.ceil(cursorX * WORKSPACES_SHOWN / widgetWidth);
