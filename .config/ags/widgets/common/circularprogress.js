...
export const AnimatedCircProg = ({
    ...
    initAnimPoints = 1,
    extraSetup = () => {  },
    startAngle = 0, // In degrees
    endAngle = 360, // In degrees
    ...rest
}) => Widget.DrawingArea({
    ...rest,
    ...
    setup: (area) => {
        ...
        area.connect('draw', Lang.bind(area, (area, cr) => {
            ...
            const start_angle = -Math.PI / 2.0 + deg2rad(startAngle);
            const end_angle = -Math.PI / 2.0 + deg2rad(endAngle);
            const progress_end_angle = -Math.PI / 2.0 + deg2rad(endAngle) * progressValue;
            ...
            const progress_end_x = center_x + Math.cos(progress_end_angle) * radius;
            const progress_end_y = center_y + Math.sin(progress_end_angle) * radius;
            ...
            // Rounded end
            cr.arc(progress_end_x, progress_end_y, fg_stroke / 2, 0, 0 - 0.01);
            cr.fill();
            ...
            cr.arc(center_x, center_y, radius, start_angle, progress_end_angle);
